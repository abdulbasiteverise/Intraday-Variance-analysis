...
/* Run-Rate: projects EOD using actual pace for elapsed intervals
 * Formula:
 *   intervalsElapsed  = number of intervals with actual data
 *   ratePerInterval   = cumActual / intervalsElapsed
 *   remainingIntervals = N - intervalsElapsed
 *   RunRate EOD       = cumActual + (ratePerInterval × remainingIntervals)
 *
 * This separates run-rate from historical completion%, so the three
 * forecast models (Hist Proj, Run-Rate, Blended) stay distinct when
 * actual pace diverges from historical pattern.
 */
function calcRunRate(cumActual, histCompletion) {
  // Pre-count how many intervals have data at each point
...
function renderNarrative(summary) {
  ...
  el.innerHTML = `${lines}
    ...
    <div class="n-line"><span class="n-dot" style="background:var(--purple-600)"></span><span>${summary.interp}</span></div>
  ...
  cDeviation = new Chart(document.getElementById('deviationChart'), {
    ...
    data: { labels: INTERVALS, datasets: [
      { label: 'Deviation %', data: new Array(N).fill(null),
        backgroundColor: new Array(N).fill('rgba(59,130,246,.4)'),
...
function chartTooltip() {
  return { backgroundColor: 'rgba(13,21,38,.92)', borderColor: 'rgba(37,99,235,.25)', borderWidth: 1, titleColor: '#f1f5f9', bodyColor: '#94a3b8', padding: 10, cornerRadius: 8 };
}
