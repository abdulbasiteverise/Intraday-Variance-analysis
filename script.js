...
/* ─────────────────────────────────────────────────────────
   §1  INTERVALS — dynamic, driven by config controls
───────────────────────────────────────────────────────── */
function genIntervals(startH, endH, stepMins = 15) {
  const out = [];
  let cur = startH * 60;
  const end = endH * 60;
  while (cur <= end) {
...
// Mutable — re-generated when config changes
let INTERVALS = genIntervals(8, 23, 15);
let N = INTERVALS.length;
const DEFAULT_WEEK_COUNT = 5;

/* Rebuild INTERVALS from the config UI controls */
function rebuildIntervals() {
  const startVal = document.getElementById('cfgStartTime')?.value || '08:00';
  const endVal   = document.getElementById('cfgEndTime')?.value   || '23:00';
  const stepVal  = parseInt(document.getElementById('cfgStep')?.value || '15', 10);

  const [sH, sM] = startVal.split(':').map(Number);
  const [eH, eM] = endVal.split(':').map(Number);
  const startMins = sH * 60 + (sM || 0);
  const endMins   = eH * 60 + (eM || 0);
  if (endMins <= startMins || stepVal < 5) { showToast('Invalid interval config'); return; }

  const prev = state.rows.map(r => ({ days: [...(r.days || [])], today: r.today }));
  INTERVALS = [];
  let cur = startMins;
  while (cur <= endMins) {
    const h = Math.floor(cur / 60);
    const m = cur % 60;
    INTERVALS.push(`${h}:${String(m).padStart(2, '0')}`);
    if (cur === endMins) break;
    cur += stepVal;
  }
  N = INTERVALS.length;

  // Re-init rows preserving as many values as possible
  state.rows = INTERVALS.map((_, i) => ({
    days: state.weeksData.map((_, d) => prev[i]?.days[d] ?? null),
    today: prev[i]?.today ?? null,
  }));
  state.weeksData.forEach(wk => { wk.values = state.rows.map(r => r.days[state.weeksData.indexOf(wk)]); });

  renderWeeks();
  buildTable();
  scheduleRecalc();
  showToast(`Intervals updated: ${N} slots × ${stepVal} min`);
}
...
/* ─────────────────────────────────────────────────────────
   §25  ADVANCED REFORECAST BRAIN
   Implements the 8-step intelligent reforecast engine:
   Gap → Classify Cause → Recovery Logic → Smart Filter → Final EOD
───────────────────────────────────────────────────────── */
function calcAdvancedReforecast(cumActual, histCompletion, histAvgs, dailyForecast, backlog) {
  if (!dailyForecast) return null;
  ...
  const A   = cumActual[lastIdx];          // Actual to date
  const E   = histCompletion[lastIdx] || 0; // Expected completion %
  const F   = dailyForecast;
  const timePct = (lastIdx + 1) / N;

  // Step 1: Expected volume at this point
  const expectedVolume = F * E;

  // Step 2: Gap
  const gap = A - expectedVolume;
  const gapPct = expectedVolume > 0 ? (gap / expectedVolume) * 100 : 0;

  // Step 3: Direction
  const behind = gap < 0;
  const absGap = Math.abs(gap);

  // Step 7: Smart filter — ignore noise
  if (Math.abs(gapPct) < 3 && timePct < 0.4) {
    return {
      scenario: 'On Track', cause: 'Within normal variance range',
      recoveryPct: null, adjustment: 0, finalForecast: F,
      gapPct, behind, filtered: true, timePct, gap,
    };
  }

  // Step 4: Classify cause
  let cause, recoveryPct;
  const lastBacklogStatus = backlog?.reduce((v, x) => x ? x.status : v, 'safe') || 'safe';
  const hasBacklog = lastBacklogStatus === 'crit' || lastBacklogStatus === 'risk';

  // Check for spike pattern (sudden jump without continuation)
  let recentDevs = [];
  for (let i = Math.max(0, lastIdx - 3); i <= lastIdx; i++) {
    const h = histAvgs[i], t = state.rows[i].today;
    if (h && t) recentDevs.push(((t - h) / h) * 100);
  }
  const avgRecentDev = recentDevs.length ? recentDevs.reduce((s, v) => s + v, 0) / recentDevs.length : 0;
  const isSpike = !behind && recentDevs.length >= 2 &&
                  recentDevs[recentDevs.length - 1] > 15 &&
                  (recentDevs.length < 2 || recentDevs[recentDevs.length - 2] < 5);

  if (behind) {
    if (hasBacklog) {
      cause = 'Delayed Demand — queue buildup present';
      recoveryPct = 0.70; // Normal delay: 60-80%
    } else if (timePct < 0.3) {
      cause = 'Uncertain — too early to classify (possible system issue)';
      recoveryPct = 0.85; // System issue: 90-100%
    } else {
      cause = 'Missing Demand — no backlog, likely genuine volume loss';
      recoveryPct = 0.30; // Missing: 20-40%
    }
  } else {
    if (isSpike) {
      cause = 'Artificial Spike — sudden jump, likely transient';
      recoveryPct = 0.25; // Spike carry: 20-30%
    } else {
      cause = 'Real Demand Shift — sustained above-trend arrivals';
      recoveryPct = 0.60; // Real shift: 50-70%
    }
  }

  // Step 5 + 6: Recovery logic → Final forecast
  let adjustment, finalForecast;
  if (behind) {
    const recovered = absGap * recoveryPct;
    adjustment = -(absGap - recovered); // negative = reduction vs F
    finalForecast = F + adjustment; // F adjusted down for unrecoverable
  } else {
    const carry = gap * recoveryPct;
    adjustment = carry;
    finalForecast = F + carry;
  }

  // Step 8: Insight output object
  return {
    scenario: behind ? 'Behind Expected' : 'Ahead of Expected',
    cause,
    recoveryPct: Math.round(recoveryPct * 100),
    ...
    actualVolume: Math.round(A),
  };
}

/* ─────────────────────────────────────────────────────────
   §26  CURRENT INTERVAL DETECTION + ROW HIGHLIGHT
───────────────────────────────────────────────────────── */
function getCurrentIntervalIndex() {
  ...
  INTERVALS.forEach((iv, i) => {
    const [h, m] = iv.split(':').map(Number);
    const ivMins = h * 60 + m;
    const diff = Math.abs(curMins - ivMins);
...
function highlightCurrentInterval() {
  ...
  document.querySelectorAll('.row-current').forEach(r => r.classList.remove('row-current'));
  const idx = getCurrentIntervalIndex();
  if (idx < 0) return;
  const row = document.getElementById(`row-${idx}`);
  if (!row) return;
  row.classList.add('row-current');
  // Auto-scroll into view if in viewport
  row.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
}

/* ─────────────────────────────────────────────────────────
   §27  CUMULATIVE ARRIVAL % KPI
   Formula: (total actual so far / daily forecast) × 100
───────────────────────────────────────────────────────── */
function calcCumulativeArrivalPct(cumActual, dailyForecast) {
  if (!dailyForecast) return null;
  let lastCum = null;
  for (let i = N - 1; i >= 0; i--) {
    if (cumActual[i] !== null) { lastCum = cumActual[i]; break; }
  }
  if (lastCum === null) return null;
  return (lastCum / dailyForecast) * 100;
}

function getArrivalPctInsight(pct, timePct) {
  if (pct === null || timePct === null) return null;
  const expectedPct = timePct * 100;
  const delta = pct - expectedPct;
  if (delta > 8)  return { label: 'Front-Loaded', cls: 'sky',    msg: 'Arrivals running ahead of expected pace — front-loaded pattern' };
  if (delta < -8) return { label: 'Behind Pace',  cls: 'rose',   msg: 'Arrivals running behind expected pace — risk of late-day pressure' };
  if (delta > 3)  return { label: 'Slightly Ahead', cls: 'em',   msg: 'Slightly ahead of arrival pace — monitor for continuation' };
  if (delta < -3) return { label: 'Slightly Behind', cls: 'am',  msg: 'Slightly behind expected pace — watch for acceleration' };
  return           { label: 'On Pace',    cls: 'em',    msg: 'Arrivals tracking expected pace — pattern normal' };
}
...
function runRecalc() {
  ...
  // §25 Advanced reforecast brain
  analysis.advancedReforecast = calcAdvancedReforecast(cumActual, histCompletion, histAvgs, dailyForecast, backlog);

  // §27 Cumulative arrival %
  const lastDataIdx = cumActual.reduce((v, x, i) => x !== null ? i : v, -1);
  const timePct = lastDataIdx >= 0 ? (lastDataIdx + 1) / N : 0;
  analysis.cumulativeArrivalPct = calcCumulativeArrivalPct(cumActual, dailyForecast);
  analysis.arrivalPctInsight    = getArrivalPctInsight(analysis.cumulativeArrivalPct, timePct);
  analysis.timePct = timePct;
  ...
  // v6 extensions
  renderAdvancedReforecastPanel(analysis);
  renderCumulativeArrivalKPI(analysis);
...
/* ═══════════════════════════════════════════════════════════
   INTELLIGENT WFM COPILOT ENGINE
   ─ Deep rule-based analysis engine (works in all browsers)
   ─ Optional: Claude API via claude.ai proxy when available
   ═══════════════════════════════════════════════════════════ */
...
function buildCopilotSnapshot() {
  ...
  const lastIdx = a.cumActual.reduce((v, x, i) => x !== null ? i : v, -1);
  const timePct = lastIdx >= 0 ? ((lastIdx + 1) / N) * 100 : 0;

  let latRF = null, latBl = null, latRR = null;
  for (let i = N - 1; i >= 0; i--) { if (a.reforecast[i] !== null) { latRF = a.reforecast[i]; break; } }
  for (let i = N - 1; i >= 0; i--) { if (a.blended[i]   !== null) { latBl = a.blended[i];   break; } }
  for (let i = N - 1; i >= 0; i--) { if (a.runRate[i]    !== null) { latRR = a.runRate[i];    break; } }

  const cumActual   = lastIdx >= 0 ? a.cumActual[lastIdx]  : null;
  const cumExpected = lastIdx >= 0 ? a.cumHist[lastIdx]    : null;
  const backlog     = a.backlog.reduce((v, x) => x ? x : v, null);
  const conf        = a.confidence.reduce((v, x) => x ? x : v, null);

  // Reforecast vs forecast delta
  const rfDelta = (latRF && a.dailyForecast)
    ? ((latRF - a.dailyForecast) / a.dailyForecast * 100) : null;

  // AHT-proxy: if spike present but volume deviation is low → AHT issue
  const spikeLevel = a.spikeSummary?.level || 'none';
  const ov = a.overallVariance;

  return {
    ov, timePct, cumActual, cumExpected, latRF, latBl, latRR,
    dailyForecast: a.dailyForecast, availCap: a.availCap,
    trafficPattern: a.trafficPattern?.label || '—',
    spikeLevel, backlog, conf, rfDelta,
    accuracy: a.accuracy, variability: a.variability,
    riskScore: typeof calcRiskScore === 'function' ? calcRiskScore(a) : null,
    periods: a.periods,
    spike: a.spike, dip: a.dip,
    lastInterval: lastIdx >= 0 ? INTERVALS[lastIdx] : null,
    // v6 additions
    advRf: a.advancedReforecast || null,
    cumulativeArrivalPct: a.cumulativeArrivalPct || null,
    arrivalPctInsight: a.arrivalPctInsight || null,
    intervalConfig: { start: INTERVALS[0], end: INTERVALS[N - 1], count: N },
  };
}

/* ── 2. INTELLIGENT LOCAL ENGINE ────────────────────────── */
function intelligentWFMResponse(userMsg, snap) {
  ...
  if (!snap) {
    return [
      ...
      '• Click **Load Example** to see a demo with spike detection',
      '• Paste your historical weekly call volumes into the input columns',
      '• Enter today\'s actual call volumes by interval',
      '',
      'Once data is loaded, I can give you a full intraday analysis covering variance, reforecast, risk, and recommended actions.',
    ].join('\n');
  }

  const { ov, timePct, cumActual, cumExpected, latRF, latBl, dailyForecast,
          availCap, trafficPattern, spikeLevel, backlog, conf, rfDelta,
          accuracy, variability, riskScore, periods, lastInterval } = snap;

  const fmt  = v => v !== null && v !== undefined ? Math.round(v).toLocaleString() : 'N/A';
  const fmtP = v => v !== null && v !== undefined ? `${v > 0 ? '+' : ''}${Number(v).toFixed(1)}%` : 'N/A';
  const absOv = Math.abs(ov || 0);
  const isHigh = absOv > 10 || spikeLevel === 'critical' || spikeLevel === 'major';
  const isMed  = absOv > 5  || spikeLevel === 'mild' || backlog?.status === 'risk';
  const isEarly = timePct < 30;
  const isLate  = timePct > 70;
  const isOver  = (ov || 0) > 0;

  // ── Route by question intent
  const isAboutRisk       = /risk|danger|sla|service level|critical/i.test(q);
  const isAboutVTO        = /vto|voluntary|send home|overstaf/i.test(q);
  const isAboutOT         = /overtime|ot|extra staff|more agent/i.test(q);
  const isAboutRecover    = /recover|catch up|make up|get back|bounce back/i.test(q);
  const isAboutSpike      = /spike|surge|jump|surge|burst|sudden/i.test(q);
  const isAboutForecast   = /forecast|reforecast|projection|eod|end.of.day|predict/i.test(q);
  const isAboutAHT        = /aht|handle time|talk time|efficiency|slow/i.test(q);
  const isAboutPattern    = /pattern|traffic|front.load|delayed|midday|evening|double/i.test(q);
  const isAboutNow        = /what.*(do|should|now|next|action|recommend)/i.test(q) || /now|action|do/i.test(q);
  const isGeneral         = /status|summary|overview|situation|how.*(are|is|look)/i.test(q);

  // ── BUILD FULL STRUCTURED RESPONSE
  const lines = [];

  const tone = isHigh ? '🚨' : isMed ? '⚠️' : '✅';
  const toneLabel = isHigh ? 'URGENT' : isMed ? 'ADVISORY' : 'STABLE';

  // --- SITUATION SUMMARY (always)
  lines.push(`${tone} **[${toneLabel}] Situation Summary**`);
  if (!dailyForecast) {
    lines.push(`No daily forecast entered yet — analysis is based on pattern deviation only. Volume is running ${fmtP(ov)} vs historical average with ${timePct.toFixed(0)}% of the day elapsed.`);
  } else {
    lines.push(
      `${timePct.toFixed(0)}% of the day has elapsed. ` +
      `Actual volume to date: **${fmt(cumActual)}** vs expected **${fmt(cumExpected)}** — a variance of **${fmtP(ov)}**. ` +
      (latRF ? `Reforecast EOD: **${fmt(latRF)}**${rfDelta !== null ? ` (${fmtP(rfDelta)} vs forecast of ${fmt(dailyForecast)})` : ''}.` : '')
    );
  }
  lines.push('');
  ...
  const causes = [];
  if (absOv > 10) causes.push(`1. Volume deviation of ${fmtP(ov)} — significantly above/below historical pattern`);
  else if (absOv > 5) causes.push(`1. Moderate volume drift of ${fmtP(ov)} from historical baseline`);
  if (spikeLevel !== 'none') causes.push(`${causes.length+1}. ${spikeLevel.charAt(0).toUpperCase()+spikeLevel.slice(1)} spike detected at ${snap.lastInterval || 'recent interval'} — positive acceleration in arrivals`);
  if (backlog?.status === 'crit') causes.push(`${causes.length+1}. Backlog critical — remaining calls exceed remaining capacity`);
  else if (backlog?.status === 'risk') causes.push(`${causes.length+1}. Backlog at risk — capacity may fall short of remaining volume`);
  if (rfDelta !== null && Math.abs(rfDelta) > 8) causes.push(`${causes.length+1}. Reforecast divergence of ${fmtP(rfDelta)} from daily forecast — actuals trending away from plan`);
  if (trafficPattern !== 'Normal Pattern' && trafficPattern !== '—') causes.push(`${causes.length+1}. Non-standard traffic pattern: ${trafficPattern} — staffing may not match this shape`);
  if (variability?.cls === 'var-high') causes.push(`${causes.length+1}. High historical variability (${variability.pct?.toFixed(1)}%) — forecast reliability is low`);
  if (isEarly) causes.push(`${causes.length+1}. Only ${timePct.toFixed(0)}% of day elapsed — early-day variance is naturally less reliable`);
  if (causes.length === 0) causes.push('No significant root causes detected — variance is within normal operating range');
  ...
  if (isHigh && isOver) {
    lines.push(`If no action is taken: ${availCap ? `with ${fmt(availCap)} daily capacity, backlog will likely accumulate in the ${isEarly ? 'midday' : 'afternoon'} window. ` : ''}SLA is at risk of dropping below target. Each 15-minute delay in response compounds queue buildup by ~10–15%.`);
  } else if (isHigh && !isOver) {
    lines.push(`Volume is running below forecast. Without action: excess idle time, possible adherence issues, and budget waste from overstaffing. If this trend holds, EOD will be ~${latRF ? fmt(latRF) : 'below'} vs forecast of ${fmt(dailyForecast)}.`);
  } else if (isMed) {
    lines.push(`Moderate variance is manageable if monitored. If it accelerates over the next 3–4 intervals, staffing pressure will increase. Recommend a soft reforecast now to stay ahead.`);
  } else {
    lines.push(`Current trajectory is stable. With ${timePct.toFixed(0)}% of the day elapsed and variance at ${fmtP(ov)}, no significant SLA impact is projected without a sudden shift.`);
  }
  lines.push('');

  // --- RECOMMENDED ACTIONS
  lines.push('⚡ **Recommended Actions**');
  lines.push('**Immediate (0–30 min):**');

  if (isOver && isHigh) {
    lines.push('• Pull agents from all AUX/offline activities immediately');
    lines.push('• Offer voluntary OT — target agents within 1hr of shift end');
    lines.push('• Reallocate cross-skilled agents from lower-volume queues');
    lines.push('• Communicate urgency to floor supervisors and team leads');
  } else if (isOver && isMed) {
    lines.push('• Soft-pull from non-critical offline activities (coaching, admin)');
    lines.push('• Alert team leads to reduce discretionary AUX time');
    lines.push('• Begin OT pre-approval process — don\'t wait until it\'s critical');
  } else if (!isOver && isHigh) {
    lines.push('• Offer VTO to willing agents — start with agents approaching overtime');
    lines.push('• Move available agents to backlog, email queues, or coaching tasks');
    lines.push('• Consider releasing agents for scheduled off-phone time early');
  } else {
    lines.push('• Continue monitoring — no immediate staffing action required');
    lines.push('• Verify break/lunch schedule alignment for remaining day');
  }

  lines.push('');
  lines.push('**Short-term (1–3 hrs):**');
  if (isOver) {
    lines.push('• Reforecast EOD using current run-rate as primary signal');
    if (rfDelta !== null && rfDelta > 5) lines.push(`• Reforecast suggests EOD of ${fmt(latRF)} — update ops manager with revised numbers`);
    lines.push('• Review afternoon staffing roster for capacity gaps');
    lines.push('• If spike sustains for 4+ intervals, escalate to full intraday review');
  } else if (!isOver) {
    lines.push('• Re-evaluate afternoon breaks — can stagger to cover any reversal');
    lines.push('• Check if pattern is consistent across all skills/queues');
    lines.push('• Monitor for potential late-day bounce if front-loaded day');
  } else {
    lines.push('• Reassess at next 30-minute checkpoint');
    lines.push('• Ensure reforecast is updated if variance changes by >3%');
  }
  lines.push('');

  // --- TRADE-OFFS
  lines.push('⚠️ **Trade-offs & Risks**');
  if (isEarly) {
    lines.push('• Acting too early (day <30%) risks overreaction — early spikes often self-correct by midday');
    lines.push('• Pulling OT at 8:30am could create excess afternoon capacity and unnecessary cost');
  }
  if (isOver && isHigh) {
    lines.push('• Offering OT without budget approval may cause compliance issues');
    lines.push('• Pulling from offline activities may delay scheduled coaching and QA reviews');
    lines.push('• Cross-skill reallocation may temporarily lower first-contact resolution on receiving queues');
  } else if (!isOver) {
    lines.push('• Sending agents home (VTO) removes buffer if volume reverses in the afternoon');
    lines.push('• Overly aggressive VTO can create a second under-staffing problem later in the day');
  }
  if (variability?.cls === 'var-high') {
    lines.push('• High historical variability means any projection has wider error margins — weight run-rate more than historical pattern');
  }
  lines.push('');

  // --- CONFIDENCE
  lines.push('🎯 **Confidence Level**');
  const confLevel = isEarly ? 'Low–Medium' : isLate ? 'High' : (absOv < 5 ? 'High' : 'Medium');
  const confReason = isEarly
    ? `Only ${timePct.toFixed(0)}% of the day has passed — insufficient data for high confidence`
    : isLate
    ? `${timePct.toFixed(0)}% elapsed, pattern well established`
    : absOv < 5
    ? 'Variance within normal range, stable trajectory'
    : 'Significant variance detected but mid-day uncertainty remains';
  lines.push(`**${confLevel}** — ${confReason}.`);

  // ── TOPIC-SPECIFIC ADDITIONS
  if (isAboutForecast && latRF) {
    lines.push('');
    lines.push('📋 **Reforecast Detail**');
    lines.push(`Current reforecast EOD: **${fmt(latRF)}**${dailyForecast ? ` vs original forecast of ${fmt(dailyForecast)} (${fmtP(rfDelta)})` : ''}`);
    lines.push(`Blended model EOD: **${fmt(latBl)}** | Run-rate EOD: **${fmt(snap.latRR)}**`);
    lines.push('The reforecast formula (CumActual + DailyForecast × Remaining %) blends your real arrivals with the historical shape of the remaining day.');
  }

  if (isAboutPattern) {
    lines.push('');
    lines.push('📈 **Traffic Pattern Detail**');
    lines.push(`Detected pattern: **${trafficPattern}**`);
    const pd = periods;
    if (pd) {
      if (pd.morning?.avg !== null) lines.push(`• Morning (8–11): ${fmtP(pd.morning.avg)} vs historical`);
      if (pd.midday?.avg  !== null) lines.push(`• Midday (11–14): ${fmtP(pd.midday.avg)} vs historical`);
      if (pd.afternoon?.avg!==null) lines.push(`• Afternoon (14–18): ${fmtP(pd.afternoon.avg)} vs historical`);
      if (pd.evening?.avg !== null) lines.push(`• Evening (18–23): ${fmtP(pd.evening.avg)} vs historical`);
    }
  }

  if (isAboutAHT) {
    lines.push('');
    lines.push('⏱️ **AHT Note**');
    lines.push('This tool tracks volume variance rather than AHT directly. However: if volume is within range but SLA is still dropping, AHT inflation is a likely cause. Key signals to investigate: rising average speed of answer, longer after-call work, or new agents handling complex call types. Recommend QA review of calls in affected intervals.');
  }

  // ── Always inject advanced reforecast brain when data is meaningful
  if (snap.advRf && !snap.advRf.filtered) {
    const rf = snap.advRf;
    lines.push('');
    lines.push('🔮 **Advanced Reforecast Intelligence**');
    lines.push(`Scenario: **${rf.scenario}** | Gap: ${rf.gap >= 0 ? '+' : ''}${rf.gap?.toLocaleString()} calls (${rf.gapPct?.toFixed(1)}%)`);
    lines.push(`Root cause: *${rf.cause}*`);
    lines.push(`Recovery factor: **${rf.recoveryPct}%** → Forecast adjustment: ${rf.adjustment >= 0 ? '+' : ''}${rf.adjustment?.toLocaleString()} calls`);
    lines.push(`**Intelligent EOD Forecast: ${rf.finalForecast?.toLocaleString()} calls**`);
    lines.push(rf.behind
      ? `→ Volume is ${Math.abs(rf.gap).toLocaleString()} calls behind. ~${rf.recoveryPct}% likely to recover (${rf.cause.includes('Delayed') ? 'queue backlog signals present' : rf.cause.includes('System') ? 'possible system/ops issue' : 'no backlog — likely genuine shortfall'}).`
      : `→ Volume is ${rf.gap.toLocaleString()} calls ahead. Carrying ${rf.recoveryPct}% forward (${rf.cause.includes('Spike') ? 'classified as transient spike' : 'classified as sustained demand shift'}).`
    );
  } else if (snap.advRf?.filtered) {
    lines.push('');
    lines.push('🔮 **Reforecast:** On track — gap within ±3%, no adjustment applied. Holding original forecast.');
  }

  return lines.join('\n');
}

/* ── 3. CLAUDE API ATTEMPT (with CORS-safe approach) ────── */
const copilotHistory = []; // session memory

async function tryClaudeAPI(userMsg, snap) {
  // Build rich system context
  const ctx = snap ? `
LIVE OPERATIONAL DATA (use this as your primary source):
- Volume variance vs historical: ${snap.ov !== null ? snap.ov.toFixed(2) + '%' : 'N/A'}
- Time elapsed today: ${snap.timePct.toFixed(1)}%
- Cumulative actual calls: ${snap.cumActual !== null ? Math.round(snap.cumActual) : 'N/A'}
- Expected by now: ${snap.cumExpected !== null ? Math.round(snap.cumExpected) : 'N/A'}
- Reforecast EOD: ${snap.latRF !== null ? Math.round(snap.latRF) : 'N/A'}
- Blended EOD forecast: ${snap.latBl !== null ? Math.round(snap.latBl) : 'N/A'}
- Daily forecast: ${snap.dailyForecast || 'not set'}
- Available daily capacity: ${snap.availCap || 'not set'}
- Traffic pattern: ${snap.trafficPattern}
- Spike level: ${snap.spikeLevel}
- Backlog status: ${snap.backlog?.status || 'safe'}
- Forecast confidence: ${snap.conf?.level || 'N/A'}
- Forecast accuracy: ${snap.accuracy?.accuracy?.toFixed(1) || 'N/A'}%
- Historical variability: ${snap.variability?.label || 'N/A'}
- Risk score: ${snap.riskScore !== null ? snap.riskScore : 'N/A'}/100
- Reforecast vs forecast delta: ${snap.rfDelta !== null ? snap.rfDelta.toFixed(1) + '%' : 'N/A'}
- Last interval with data: ${snap.lastInterval || 'N/A'}
- Period deviations: Morning ${snap.periods?.morning?.avg?.toFixed(1) || 'N/A'}%, Midday ${snap.periods?.midday?.avg?.toFixed(1) || 'N/A'}%, Afternoon ${snap.periods?.afternoon?.avg?.toFixed(1) || 'N/A'}%, Evening ${snap.periods?.evening?.avg?.toFixed(1) || 'N/A'}%
- Cumulative arrival %: ${snap.cumulativeArrivalPct !== null ? snap.cumulativeArrivalPct?.toFixed(1) + '%' : 'N/A'} (${snap.arrivalPctInsight?.label || 'N/A'})
${snap.advRf && !snap.advRf.filtered ? `
ADVANCED REFORECAST BRAIN (8-step intelligent engine):
- Scenario: ${snap.advRf.scenario}
- Gap vs expected: ${snap.advRf.gap >= 0 ? '+' : ''}${snap.advRf.gap} calls (${snap.advRf.gapPct?.toFixed(1)}%)
- Classified cause: ${snap.advRf.cause}
- Recovery factor: ${snap.advRf.recoveryPct}%
- Forecast adjustment: ${snap.advRf.adjustment >= 0 ? '+' : ''}${snap.advRf.adjustment} calls
- Intelligent EOD forecast: ${snap.advRf.finalForecast?.toLocaleString()} calls
- Reasoning: ${snap.advRf.behind ? 'Volume behind expected — apply recovery factor to determine unrecoverable shortfall' : 'Volume ahead — apply carry factor to determine sustained uplift'}
` : snap.advRf?.filtered ? '- Advanced reforecast: On track (gap <3%, no adjustment needed)' : ''}
` : 'No data loaded yet.';

  const systemPrompt = `You are an elite Workforce Management (WFM) Copilot with 10+ years of real-world experience as an intraday analyst and RTA specialist at large contact centers. You think like a senior WFM lead who has seen every intraday scenario.

RESPONSE FORMAT — always use ALL 6 sections:
1. 🧠 **Situation Summary** (2–3 sentences, direct)
2. 🔍 **Root Cause Analysis** (ranked list, most likely first)
3. 📉 **Impact Projection** (quantified where possible)
4. ⚡ **Recommended Actions**
   - **Immediate (0–30 min):** (bullet points)
   - **Short-term (1–3 hrs):** (bullet points)
5. ⚠️ **Trade-offs & Risks** (honest assessment)
6. 🎯 **Confidence Level** (High/Medium/Low with specific reason)

TONE RULES:
- Risk > 60 or spike = critical/major → urgent, imperative language, no hedging
- Risk 35–60 or spike = mild → advisory tone, balanced
- Time < 30% → explicitly flag early-day caution
- No data → guide them to load data first

WFM EXPERTISE TO APPLY:
- Know Erlang C implications: small staffing changes have outsized SLA impact at high occupancy
- Distinguish volume-driven vs AHT-driven SLA pressure
- Know when NOT to act (early day, within ±3% variance, high variability)
- Understand reforecast vs run-rate differences and when each is more reliable
- Use real WFM language: AUX, adherence, occupancy, shrinkage, VTO, OT, interval, reforecast

${ctx}`;

  const messages = [
    ...copilotHistory.slice(-8), // keep last 4 turns
  ...
  // Save to memory
  copilotHistory.push({ role: 'user', content: userMsg });
...
function initCopilot() {
  ...
  const msgsEl   = document.getElementById('copilotMessages');
  const inputEl  = document.getElementById('copilotInput');
  const sendEl   = document.getElementById('copilotSend');

  if (!panel || !toggle || !msgsEl || !inputEl || !sendEl) return;

  // ── Render a message bubble
  function renderMsg(html, who) {
  ...
  // ── Format text: markdown-lite → HTML
  function fmt(text) {
    return text
  ...
  function showLoading() {
    const m = document.createElement('div');
    m.className = 'copilot-msg copilot-msg--ai';
  ...
  function renderChips() {
    const chips = [
      { label: '📊 Status update',    q: 'Give me a full status update on the current situation' },
      { label: '⚡ What should I do?', q: 'What actions should I take right now?' },
      { label: '🔮 Can we recover?',   q: 'Can we recover SLA by end of day?' },
      { label: '📉 Offer VTO?',        q: 'Should I offer VTO to agents?' },
      { label: '📈 Offer OT?',         q: 'Should I offer overtime to agents?' },
      { label: '🧠 Explain variance',  q: 'Why is there a variance? What is causing it?' },
    ];
    const row = document.createElement('div');
    row.className = 'copilot-chips';
    row.style.cssText = 'display:flex;flex-wrap:wrap;gap:6px;padding:8px 0 4px;';
    chips.forEach(({ label, q }) => {
      const btn = document.createElement('button');
      btn.textContent = label;
      btn.style.cssText = 'font-size:11px;font-family:inherit;padding:4px 10px;border-radius:20px;border:1px solid rgba(56,189,248,.25);background:rgba(56,189,248,.08);color:#38bdf8;cursor:pointer;transition:all .18s ease;white-space:nowrap;';
      btn.onmouseover = () => { btn.style.background = 'rgba(56,189,248,.18)'; };
      btn.onmouseout  = () => { btn.style.background = 'rgba(56,189,248,.08)'; };
      btn.onclick = () => { inputEl.value = q; handleSend(); };
      row.appendChild(btn);
    });
  ...
  async function handleSend() {
    ...
    try {
      // Try Claude API first (works if deployed with proxy / claude.ai)
      const reply = await tryClaudeAPI(q, snap);
      loader.remove();
      renderMsg(fmt(reply), 'ai');
    } catch (_) {
      // Seamless fallback to intelligent local engine
      const reply = intelligentWFMResponse(q, snap);
      loader.remove();
      renderMsg(fmt(reply), 'ai');
    }
  ...
  msgsEl.innerHTML = '';
  renderMsg(fmt([
    '👋 **WFM AI Copilot** — your intraday intelligence assistant.',
    '',
    'I analyse your live data and respond like a senior WFM lead with 10+ years of intraday experience.',
    '',
    'Try asking:',
    '• "What\'s the current risk level?"',
    '• "Should I offer VTO or OT?"',
    '• "Will we hit SLA today?"',
...
/* ═══════════════════════════════════════════════════════════════
   v6 ENHANCEMENTS
   §31  Advanced Reforecast Panel
   §32  Cumulative Arrival KPI
   §33  Interval Config UI wiring
   §34  Current Interval Auto-highlight
   §35  Copilot Reforecast Brain integration
   ═══════════════════════════════════════════════════════════════ */

/* ── §31 ADVANCED REFORECAST PANEL ───────────────────────── */
function renderAdvancedReforecastPanel(a) {
  // Update interval count display
  const cntEl = document.getElementById('cfgIntervalCount');
  if (cntEl) cntEl.textContent = N;

  // Update intelligent EOD KPI tiles
  const advRfKpiEl  = document.getElementById('siAdvRfForecast');
  const advRfScnEl  = document.getElementById('siAdvRfScenario');
  const rf = a.advancedReforecast;

  if (advRfKpiEl) {
    if (rf) {
      advRfKpiEl.textContent = rf.finalForecast?.toLocaleString() || '—';
      advRfKpiEl.style.color = rf.behind ? 'var(--rose)' : 'var(--em)';
    } else {
      advRfKpiEl.textContent = '—';
      advRfKpiEl.style.color = '';
    }
  }
  if (advRfScnEl) {
    advRfScnEl.textContent = rf ? rf.filtered ? 'On Track' : rf.scenario : '';
    advRfScnEl.style.color = rf ? (rf.behind ? 'var(--rose)' : 'var(--em)') : '';
  }

  const el = document.getElementById('advReforecastPanel');
  if (!el) return;
  if (!rf) {
    el.innerHTML = `<div class="adv-rf-empty">Enter daily forecast + today's actuals to activate intelligent reforecast</div>`;
    return;
  }
  const clr = rf.behind ? 'var(--rose)' : 'var(--em)';
  const icon = rf.filtered ? '✅' : rf.behind ? '📉' : '📈';
  const sign = rf.adjustment >= 0 ? '+' : '';

  el.innerHTML = `
    <div class="adv-rf-grid">
      <div class="adv-rf-cell">
        <div class="adv-rf-label">Scenario</div>
        <div class="adv-rf-val" style="color:${clr}">${icon} ${rf.scenario}</div>
      </div>
      <div class="adv-rf-cell">
        <div class="adv-rf-label">Gap vs Expected</div>
        <div class="adv-rf-val">${rf.gap >= 0 ? '+' : ''}${rf.gap?.toLocaleString() || '—'} calls (${rf.gapPct >= 0 ? '+' : ''}${rf.gapPct?.toFixed(1)}%)</div>
      </div>
      <div class="adv-rf-cell">
        <div class="adv-rf-label">Root Cause</div>
        <div class="adv-rf-val adv-rf-cause">${rf.cause}</div>
      </div>
      <div class="adv-rf-cell">
        <div class="adv-rf-label">Recovery Factor</div>
        <div class="adv-rf-val">${rf.recoveryPct !== null ? rf.recoveryPct + '%' : 'N/A (on track)'}</div>
      </div>
      <div class="adv-rf-cell">
        <div class="adv-rf-label">Forecast Adjustment</div>
        <div class="adv-rf-val" style="color:${clr}">${sign}${rf.adjustment?.toLocaleString() || '0'} calls</div>
      </div>
      <div class="adv-rf-cell adv-rf-cell--highlight">
        <div class="adv-rf-label">Intelligent EOD Forecast</div>
        <div class="adv-rf-val adv-rf-final">${rf.finalForecast?.toLocaleString() || '—'}</div>
      </div>
    </div>
    ${!rf.filtered ? `<div class="adv-rf-insight">💡 ${rf.behind
      ? `Volume is <strong>${Math.abs(rf.gap).toLocaleString()}</strong> calls behind expected. Based on <em>${rf.cause.toLowerCase()}</em>, approximately <strong>${rf.recoveryPct}%</strong> is likely to recover. Adjusted EOD forecast: <strong>${rf.finalForecast?.toLocaleString()}</strong>.`
      : `Volume is <strong>${rf.gap.toLocaleString()}</strong> calls ahead of expected. With a <strong>${rf.recoveryPct}%</strong> carry factor for this demand type, adjusted EOD forecast: <strong>${rf.finalForecast?.toLocaleString()}</strong>.`
    }</div>` : `<div class="adv-rf-insight">✅ Volume is within ±3% of expected and pattern is not yet persistent. Holding original forecast of <strong>${a.dailyForecast?.toLocaleString() || '—'}</strong> — no adjustment needed.</div>`}
  `;
}

/* ── §32 CUMULATIVE ARRIVAL KPI ──────────────────────────── */
function renderCumulativeArrivalKPI(a) {
  const valEl  = document.getElementById('cumArrivalPctVal');
  const lblEl  = document.getElementById('cumArrivalPctLbl');
  const insEl  = document.getElementById('cumArrivalPctIns');
  if (!valEl) return;

  const pct = a.cumulativeArrivalPct;
  const ins = a.arrivalPctInsight;

  if (pct === null) {
    valEl.textContent = '—';
    if (lblEl) lblEl.textContent = 'Cumulative Arrival %';
    if (insEl) insEl.textContent = 'Requires daily forecast';
    return;
  }

  valEl.textContent = pct.toFixed(1) + '%';
  const colorMap = { sky: 'var(--sky)', rose: 'var(--rose)', em: 'var(--em)', am: 'var(--am)' };
  valEl.style.color = colorMap[ins?.cls] || 'var(--t1)';
  if (lblEl) lblEl.textContent = 'Cumulative Arrival %';
  if (insEl) { insEl.textContent = ins?.msg || ''; insEl.style.color = colorMap[ins?.cls] || ''; }
}

/* ── §33 INTERVAL CONFIG WIRING ──────────────────────────── */
function initIntervalConfig() {
  const applyBtn = document.getElementById('cfgApplyBtn');
  if (applyBtn) applyBtn.addEventListener('click', rebuildIntervals);

  // Also wire Enter key on time inputs
  ['cfgStartTime', 'cfgEndTime', 'cfgStep'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.addEventListener('change', () => {});
  });
}

/* ── §34 CURRENT INTERVAL AUTO-REFRESH ──────────────────── */
function initCurrentIntervalHighlight() {
  highlightCurrentInterval();
  // Refresh every 60 seconds
  setInterval(highlightCurrentInterval, 60000);
}

/* ── §35 COPILOT REFORECAST BRAIN UPGRADE ────────────────── */
// Extend buildCopilotSnapshot to include advanced reforecast data
// (called by initCopilot via closure — we intercept via a wrapper on the module level)
function _v6_enrichSnapshot(snap) {
  if (!snap || !state.analysis) return snap;
  const a = state.analysis;
  snap.advRf = a.advancedReforecast;
  snap.cumulativeArrivalPct = a.cumulativeArrivalPct;
  snap.arrivalPctInsight    = a.arrivalPctInsight;
  snap.intervalConfig = {
    start: INTERVALS[0],
    end: INTERVALS[N - 1],
    count: N,
    step: N > 1 ? (() => {
      const [h1, m1] = INTERVALS[0].split(':').map(Number);
      const [h2, m2] = INTERVALS[1].split(':').map(Number);
...
// Monkey-patch: store original, replace on window so initCopilot picks up new version
const _v6_origSnap = buildCopilotSnapshot;
// Override in global scope (works outside strict-mode module boundary)
window._buildCopilotSnapshot_v6 = function() {
  return _v6_enrichSnapshot(_v6_origSnap());
};

// Extend intelligentWFMResponse to inject reforecast brain reasoning
function _v6_enrichResponse(base, snap) {
  if (!snap?.advRf || snap.advRf.filtered) return base;
  const rf = snap.advRf;
  const reforecastSection = [
    '',
    '🔮 **Advanced Reforecast Intelligence**',
    `Scenario: **${rf.scenario}** | Gap: ${rf.gap >= 0 ? '+' : ''}${rf.gap?.toLocaleString()} calls (${rf.gapPct?.toFixed(1)}%)`,
    `Root cause: *${rf.cause}*`,
    `Recovery factor: **${rf.recoveryPct}%** → Adjustment: ${rf.adjustment >= 0 ? '+' : ''}${rf.adjustment?.toLocaleString()} calls`,
    `**Intelligent EOD: ${rf.finalForecast?.toLocaleString()} calls**`,
    rf.behind
      ? `→ Volume behind. ~${rf.recoveryPct}% likely to recover based on ${rf.cause.includes('Delayed') ? 'queue/backlog signals' : rf.cause.includes('System') ? 'system issue indicators' : 'absence of backlog (genuine shortfall)'}.`
      : `→ Volume ahead. Carrying ${rf.recoveryPct}% forward based on ${rf.cause.includes('Spike') ? 'transient spike pattern' : 'sustained demand shift'}.`,
  ].join('\n');
  const marker = '⚡ **Recommended Actions**';
  const idx = base.indexOf(marker);
  return idx >= 0
    ? base.slice(0, idx) + reforecastSection + '\n\n' + base.slice(idx)
    : base + reforecastSection;
}

/* ── INIT v6 ─────────────────────────────────────────────── */
document.addEventListener('DOMContentLoaded', () => {
  initIntervalConfig();
  initCurrentIntervalHighlight();

  // Patch initCopilot's internal closures by wrapping the global functions
  // that it calls at send-time. Since buildCopilotSnapshot and intelligentWFMResponse
  // are called inside handleSend() in initCopilot(), we intercept via the
  // module's shared tryClaudeAPI which calls buildCopilotSnapshot() directly.
  // The enrichment happens at analysis.advancedReforecast which is already
  // populated in state.analysis by runRecalc — so tryClaudeAPI/intelligentWFMResponse
  // receive the full enriched snap automatically via buildCopilotSnapshot().
});
